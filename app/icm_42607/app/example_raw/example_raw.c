...
int configure_imu_device(void)
{
	...
	inv_imu_set_accel_ln_bw(&icm_driver, ACCEL_CONFIG1_ACCEL_FILT_BW_16);  //ACCEL_CONFIG1_ACCEL_FILT_BW_16  //ACCEL_CONFIG1_ACCEL_FILT_BW_NO_FILTER
	inv_imu_set_gyro_ln_bw(&icm_driver, GYRO_CONFIG1_GYRO_FILT_BW_16); //GYRO_CONFIG1_GYRO_FILT_BW_16  //GYRO_CONFIG1_GYRO_FILT_BW_NO_FILTER
...
static void get_accel_and_gyr_fsr(uint16_t * accel_fsr_g, uint16_t * gyro_fsr_dps)
{
	...
	break;
	default:                        *accel_fsr_g = (uint16_t)-1;
	}
	...
	break;
	default:                          *gyro_fsr_dps = (uint16_t)-1;
	}
}
#endif
...
void imu_callback(inv_imu_sensor_event_t *event)
{
	...
	*/
	...
	extern void put_imu_data(float *accel_g, float *gyro_dps);

	//CWM_OS_dbgPrintf("imu_callback CWM_OS_GetTimeMs = %lld\n", CWM_OS_GetTimeNs() / 1000000);
	put_imu_data(accel_g, gyro_dps);
	...
	if (event->sensor_mask & (1 << INV_SENSOR_ACCEL) && event->sensor_mask & (1 << INV_SENSOR_GYRO))
		...
		INV_MSG(INV_MSG_LEVEL_INFO, "%u: NA, NA, NA, %.3f, %.3f, %.3f, %.3f", 
		        (uint32_t)timestamp,
		        temp_degc,
		        gyro_dps[0], gyro_dps[1], gyro_dps[2]);
	else if (event->sensor_mask & (1 << INV_SENSOR_ACCEL))
		INV_MSG(INV_MSG_LEVEL_INFO, "%u: %.3f, %.3f, %.3f, %.3f, NA, NA, NA", 
		        (uint32_t)timestamp,
		        accel_g[0], accel_g[1], accel_g[2],
		        temp_degc);
#else
	 ...
	 */
	if (event->sensor_mask & (1 << INV_SENSOR_ACCEL) && event->sensor_mask & (1 << INV_SENSOR_GYRO))
		INV_MSG(INV_MSG_LEVEL_INFO, "%u: %d, %d, %d, %d, %d, %d, %d", 
		        (uint32_t)timestamp,
		        accel[0], accel[1], accel[2], 
		        event->temperature,
		        gyro[0], gyro[1], gyro[2]);
	else if (event->sensor_mask & (1 << INV_SENSOR_GYRO))
		INV_MSG(INV_MSG_LEVEL_INFO, "%u: NA, NA, NA, %d, %d, %d, %d", 
		        (uint32_t)timestamp,
		        event->temperature,
		        gyro[0], gyro[1], gyro[2]);
	else if (event->sensor_mask & (1 << INV_SENSOR_ACCEL))
		INV_MSG(INV_MSG_LEVEL_INFO, "%u: %d, %d, %d, %d, NA, NA, NA", 
		        (uint32_t)timestamp,
		        accel[0], accel[1], accel[2],
		        event->temperature);
#endif
