...
extern int CWM_OS_i2cRead(uint8_t slaveAddr, uint8_t reg, uint8_t regLength, uint8_t *readData, uint8_t readDataSize);
extern int CWM_OS_i2cWrite(uint8_t slaveAddr, uint8_t reg, uint8_t regLength, uint8_t *writeData, uint8_t writeDataSize);
...
int inv_io_hal_read_reg(struct inv_imu_serif *serif, uint8_t reg, uint8_t *rbuffer, uint32_t rlen)
{
	...
	return CWM_OS_i2cRead(ICM_I2C_ADDR, reg, 1, rbuffer, rlen);
#if 0
	switch (serif->serif_type) {
		case UI_SPI4:
...
int inv_io_hal_write_reg(struct inv_imu_serif *serif, uint8_t reg, uint8_t *wbuffer, uint32_t wlen)
{
	...
	return CWM_OS_i2cWrite(ICM_I2C_ADDR, reg, 1, wbuffer, wlen);
