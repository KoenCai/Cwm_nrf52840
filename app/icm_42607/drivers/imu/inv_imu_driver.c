...
int inv_imu_device_reset(struct inv_imu_device *s)
{
	...
	/* Check status and return if an error occured */
	if (status)
		return status;
	...
	if (s->transport.serif.serif_type == UI_SPI4) {
		data = 1 << DEVICE_CONFIG_SPI_AP_4WIRE_POS;
		status |= inv_imu_write_reg(s, DEVICE_CONFIG, 1, &data);
	}
	...
	/* Make sure `need_mclk_cnt` is cleared (`reload_otp()` will need it)*/
...
int inv_imu_get_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it)
{
	...
	it->INV_UI_FSYNC  = (inv_imu_interrupt_value)((data & INT_SOURCE0_FSYNC_INT1_EN_MASK)  >> INT_SOURCE0_FSYNC_INT1_EN_POS);
	it->INV_UI_DRDY   = (inv_imu_interrupt_value)((data & INT_SOURCE0_DRDY_INT1_EN_MASK)   >> INT_SOURCE0_DRDY_INT1_EN_POS);
	it->INV_FIFO_THS  = (inv_imu_interrupt_value)((data & INT_SOURCE0_FIFO_THS_INT1_EN_MASK)  >> INT_SOURCE0_FIFO_THS_INT1_EN_POS);
	it->INV_FIFO_FULL = (inv_imu_interrupt_value)((data & INT_SOURCE0_FIFO_FULL_INT1_EN_MASK) >> INT_SOURCE0_FIFO_FULL_INT1_EN_POS);
	...
	it->INV_STEP_DET      = (inv_imu_interrupt_value)((data & INT_SOURCE6_STEP_DET_INT1_EN_MASK)      >> INT_SOURCE6_STEP_DET_INT1_EN_POS);
	it->INV_STEP_CNT_OVFL = (inv_imu_interrupt_value)((data & INT_SOURCE6_STEP_CNT_OFL_INT1_EN_MASK) >> INT_SOURCE6_STEP_CNT_OFL_INT1_EN_POS);
	it->INV_TILT_DET      = (inv_imu_interrupt_value)((data & INT_SOURCE6_TILT_DET_INT1_EN_MASK)      >> INT_SOURCE6_TILT_DET_INT1_EN_POS);
...
int inv_imu_get_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it)
{
	...
	it->INV_UI_FSYNC  = (inv_imu_interrupt_value)((data & INT_SOURCE3_FSYNC_INT2_EN_MASK)  >> INT_SOURCE3_FSYNC_INT2_EN_POS);
	it->INV_UI_DRDY   = (inv_imu_interrupt_value)((data & INT_SOURCE3_DRDY_INT2_EN_MASK)   >> INT_SOURCE3_DRDY_INT2_EN_POS);
	it->INV_FIFO_THS  = (inv_imu_interrupt_value)((data & INT_SOURCE3_FIFO_THS_INT2_EN_MASK)  >> INT_SOURCE3_FIFO_THS_INT2_EN_POS);
	it->INV_FIFO_FULL = (inv_imu_interrupt_value)((data & INT_SOURCE3_FIFO_FULL_INT2_EN_MASK) >> INT_SOURCE3_FIFO_FULL_INT2_EN_POS);
	
	status |= inv_imu_read_reg(s, INT_SOURCE4, 1, &data);
	it->INV_SMD   = (inv_imu_interrupt_value)((data & INT_SOURCE4_SMD_INT2_EN_MASK)   >> INT_SOURCE4_SMD_INT2_EN_POS);
	it->INV_WOM_X = (inv_imu_interrupt_value)((data & INT_SOURCE4_WOM_X_INT2_EN_MASK) >> INT_SOURCE4_WOM_X_INT2_EN_POS);
	it->INV_WOM_Y = (inv_imu_interrupt_value)((data & INT_SOURCE4_WOM_Y_INT2_EN_MASK) >> INT_SOURCE4_WOM_Y_INT2_EN_POS);
	it->INV_WOM_Z = (inv_imu_interrupt_value)((data & INT_SOURCE4_WOM_Z_INT2_EN_MASK) >> INT_SOURCE4_WOM_Z_INT2_EN_POS);
	
	status |= inv_imu_read_reg(s, INT_SOURCE7_MREG1, 1, &data);
	it->INV_FF            = (inv_imu_interrupt_value)((data & INT_SOURCE7_FF_INT2_EN_MASK)            >> INT_SOURCE7_FF_INT2_EN_POS);
	it->INV_LOWG          = (inv_imu_interrupt_value)((data & INT_SOURCE7_LOWG_INT2_EN_MASK)          >> INT_SOURCE7_LOWG_INT2_EN_POS);
	it->INV_STEP_DET      = (inv_imu_interrupt_value)((data & INT_SOURCE7_STEP_DET_INT2_EN_MASK)      >> INT_SOURCE7_STEP_DET_INT2_EN_POS);
	it->INV_STEP_CNT_OVFL = (inv_imu_interrupt_value)((data & INT_SOURCE7_STEP_CNT_OFL_INT2_EN_MASK) >> INT_SOURCE7_STEP_CNT_OFL_INT2_EN_POS);
	it->INV_TILT_DET      = (inv_imu_interrupt_value)((data & INT_SOURCE7_TILT_DET_INT2_EN_MASK)      >> INT_SOURCE7_TILT_DET_INT2_EN_POS);
...
int inv_imu_set_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it)
{
	...
	data[0] &= ~(INT_SOURCE0_FSYNC_INT1_EN_MASK 
			   | INT_SOURCE0_DRDY_INT1_EN_MASK 
			   | INT_SOURCE0_FIFO_THS_INT1_EN_MASK 
	...
	data[1] &= ~(INT_SOURCE1_SMD_INT1_EN_MASK 
			   | INT_SOURCE1_WOM_X_INT1_EN_MASK 
			   | INT_SOURCE1_WOM_Y_INT1_EN_MASK 
	...
	data[0] &= ~(INT_SOURCE6_FF_INT1_EN_MASK
			| INT_SOURCE6_LOWG_INT1_EN_MASK
			| INT_SOURCE6_STEP_DET_INT1_EN_MASK
...
int inv_imu_set_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it)
{
	...
	data[0] &= ~(INT_SOURCE3_FSYNC_INT2_EN_MASK 
			| INT_SOURCE3_DRDY_INT2_EN_MASK 
			| INT_SOURCE3_FIFO_THS_INT2_EN_MASK 
			| INT_SOURCE3_FIFO_FULL_INT2_EN_MASK);
	...
	data[1] &= ~(INT_SOURCE4_SMD_INT2_EN_MASK 
			| INT_SOURCE4_WOM_X_INT2_EN_MASK 
			| INT_SOURCE4_WOM_Y_INT2_EN_MASK 
			| INT_SOURCE4_WOM_Z_INT2_EN_MASK);
	...
	data[0] &= ~(INT_SOURCE7_FF_INT2_EN_MASK
			| INT_SOURCE7_LOWG_INT2_EN_MASK
			| INT_SOURCE7_STEP_DET_INT2_EN_MASK
			| INT_SOURCE7_STEP_CNT_OFL_INT2_EN_MASK
...
int inv_imu_get_data_from_fifo(struct inv_imu_device *s)
{
	...
	uint16_t packet_size = FIFO_HEADER_SIZE 
	...
	if ((int_status & INT_STATUS_FIFO_THS_INT_MASK) 
		 ...
		 * If the program is interrupted for more than 1ms, the chip will switch the clock,
		...
		while (packet_count > 0) {
			...
			if ((status |= inv_imu_read_reg(s, FIFO_DATA, packet_size *packet_count, s->fifo_data))) {
				/* 
...
int inv_imu_get_accel_fsr(struct inv_imu_device *s, ACCEL_CONFIG0_FS_SEL_t *accel_fsr_g)
{
	...
	if ((s->fifo_highres_enabled) && (s->fifo_is_used == INV_IMU_FIFO_ENABLED))
		*accel_fsr_g = ACCEL_CONFIG0_FS_SEL_MAX;
		...
		status |= inv_imu_read_reg(s, ACCEL_CONFIG0, 1, &accel_config0_reg);
		*accel_fsr_g = (ACCEL_CONFIG0_FS_SEL_t)(accel_config0_reg & ACCEL_CONFIG0_ACCEL_UI_FS_SEL_MASK);
	}
...
int inv_imu_get_gyro_fsr(struct inv_imu_device *s, GYRO_CONFIG0_FS_SEL_t *gyro_fsr_dps)
{
	...
	if ((s->fifo_highres_enabled) && (s->fifo_is_used == INV_IMU_FIFO_ENABLED))
		*gyro_fsr_dps = GYRO_CONFIG0_FS_SEL_MAX;
		...
		status |= inv_imu_read_reg(s, GYRO_CONFIG0, 1, &gyro_config0_reg);
		*gyro_fsr_dps = (GYRO_CONFIG0_FS_SEL_t)(gyro_config0_reg & GYRO_CONFIG0_GYRO_UI_FS_SEL_MASK);
	}
...
int inv_imu_reset_fifo(struct inv_imu_device *s)
{
	...
	while (   (status == 0) 
		   && ((fifo_flush_status & SIGNAL_PATH_RESET_FIFO_FLUSH_MASK) 
...
int inv_imu_configure_fifo(struct inv_imu_device *s, INV_IMU_FIFO_CONFIG_t fifo_config)
{
	...
	case INV_IMU_FIFO_ENABLED :	
		...
		*/
		...
		data &= ~(INTF_CONFIG0_FIFO_COUNT_FORMAT_MASK 
		...
		data &= ~(FIFO_CONFIG1_FIFO_MODE_MASK 
		        | FIFO_CONFIG1_FIFO_BYPASS_MASK);
		data |=  (uint8_t)FIFO_CONFIG1_FIFO_MODE_SNAPSHOT 
		       | (uint8_t)FIFO_CONFIG1_FIFO_BYPASS_OFF;
		...
		data &= ~(FIFO_CONFIG5_FIFO_GYRO_EN_MASK 
		        | FIFO_CONFIG5_FIFO_ACCEL_EN_MASK
		        | FIFO_CONFIG5_FIFO_TMST_FSYNC_EN_MASK 
		        | FIFO_CONFIG5_FIFO_HIRES_EN_MASK);
		data |= ( (uint8_t)FIFO_CONFIG5_GYRO_EN 
		        | (uint8_t)FIFO_CONFIG5_ACCEL_EN 
		        | (uint8_t)FIFO_CONFIG5_TMST_FSYNC_EN 
		        | (uint8_t)FIFO_CONFIG5_WM_GT_TH_EN);
	...
	case INV_IMU_FIFO_DISABLED :
		...
		data |= ( (uint8_t)FIFO_CONFIG5_GYRO_DIS 
		        | (uint8_t)FIFO_CONFIG5_ACCEL_DIS 
		        | (uint8_t)FIFO_CONFIG5_TMST_FSYNC_EN);
		status |= inv_imu_write_reg(s, FIFO_CONFIG5_MREG1, 1, &data);
...
int inv_imu_reset_dmp(struct inv_imu_device *s, const APEX_CONFIG0_DMP_MEM_RESET_t sram_reset)
{
	...
	} while (((data_dmp_reset & APEX_CONFIG0_DMP_MEM_RESET_EN_MASK) != APEX_CONFIG0_DMP_MEM_RESET_DIS) && timeout-- && !status);
